...
def find_midi_sequences(first_index, last_index, pitch_table, base_frequency_hz):
  ...
  for midi_index in range(first_index, last_index):
    ...
    midi_sequences.append({
      ...
      "computed_error": chosen_sequence["error"],
...
if __name__ == '__main__':
    ...
    with open(output_filename, "w") as output_file:
      ...
      print(ca65_label("zsaw_note_lists"), file=output_file)
      for midi_sequence in midi_sequences:
        print("  .word zsaw_note_period_%s" % midi_sequence["midi_index"], file=output_file)
      print("")
      for midi_sequence in midi_sequences:
        print(ca65_label("zsaw_note_period_%s" % midi_sequence["midi_index"]), file=output_file)
